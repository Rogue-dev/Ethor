...
checkExperience checkDefense defense cookies*/
var active = false;
async function attack(monster) {
  if (
    rooms[currentRoom].enemies[monster] !== undefined &&
    rooms[currentRoom].enemies[monster].status === "alive"
  ) {
    if (active === false) {
      active = true;
      checkBestWeapon();
      ...
      while (!health <= 0 || !monsterHealth <= 0) {
        ...
        if (monsterHealth <= 0) {
          ...
          addText(
            "You have received a " +
              rooms[currentRoom].enemies[monster].loot +
              "!"
          );
      ...
      addText("You are already attacking something!")
    }
  } else {
